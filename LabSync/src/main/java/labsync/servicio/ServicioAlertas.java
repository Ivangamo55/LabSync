...
public final class ServicioAlertas {
    ...
    private void generarReservasPendientes(Connection conexion) throws SQLException {
        String sql = "SELECT id_reserva, nombre_solicitante, rol_solicitante, laboratorio, "
                + "actividad, fecha, horario FROM reservas WHERE estado='Pendiente' "
                + "AND fecha >= CURDATE()";
        try (PreparedStatement ps = conexion.prepareStatement(sql);
                ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
    ...
    private void generarResolucionesReserva(Connection conexion) throws SQLException {
        String sql = "SELECT id_reserva, laboratorio, fecha, horario, estado, observaciones "
                + "FROM reservas WHERE estado IN ('Aprobada','Rechazada')";
        try (PreparedStatement ps = conexion.prepareStatement(sql);
                ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
