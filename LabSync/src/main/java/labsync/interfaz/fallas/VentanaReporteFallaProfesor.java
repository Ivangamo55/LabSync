package labsync.interfaz.fallas;
 ...
 * La sección initComponents se mantiene sincronizada con VentanaReporteFallaProfesor.form.
 */
public class VentanaReporteFallaProfesor extends javax.swing.JFrame {

    private String nombreUsuario;
    private SesionUsuario sesion;

    public VentanaReporteFallaProfesor() {
        this("Profesor");
    }

    public VentanaReporteFallaProfesor(String nombreRecibido) {
        this(SesionUsuario.buscarProfesor(nombreRecibido));
    }

    public VentanaReporteFallaProfesor(SesionUsuario sesionRecibida) {
        initComponents();
    ...
    private void iniciarActualizacionAutomatica() {
        new ActualizacionAutomatica<>(this, 7_000, () -> ConsultaTabla.ejecutar(
                ...
                modelo -> { tablaReportes.setModel(modelo); ocultarPrimeraColumna(tablaReportes); });
    }
    ...
    private void cargarReportes() {
        ...
        String sql = "SELECT f.id_falla, DATE_FORMAT(f.fecha_reporte,'%d/%m/%Y') fecha, l.nombre laboratorio, "
                + "COALESCE(i.codigo,i.nombre_equipo,'Sin especificar') equipo, f.estado FROM reporte_fallas f "
                + "JOIN laboratorios l ON l.id_laboratorio=f.id_laboratorio LEFT JOIN inventario i ON i.id_inventario=f.id_inventario "
                + "WHERE f.id_usuario=? ORDER BY f.fecha_reporte DESC LIMIT 8";
        try (java.sql.Connection con = ConexionBaseDatos.conectar()) {
            if (con == null) return;
            try (java.sql.PreparedStatement ps = con.prepareStatement(sql)) {
    ...
    private void enviarReporte(){
        ...
        String sql="INSERT INTO reporte_fallas (id_usuario,id_inventario,id_laboratorio,descripcion_falla,estado,fecha_reporte) SELECT ?,i.id_inventario,l.id_laboratorio,?,'Pendiente',? FROM laboratorios l LEFT JOIN inventario i ON i.codigo=? AND i.id_laboratorio=l.id_laboratorio WHERE l.nombre=?";
        try(java.sql.Connection con=ConexionBaseDatos.conectar()){if(con==null)throw new java.sql.SQLException("No hay conexión con la base de datos.");try(java.sql.PreparedStatement ps=con.prepareStatement(sql)){ps.setInt(1,sesion.getId());ps.setString(2,"[Tipo: "+cmbTipo.getSelectedItem()+"] "+txtDescripcion.getText().trim());java.time.LocalTime hora=fecha.equals(java.time.LocalDate.now())?java.time.LocalTime.now():java.time.LocalTime.NOON;ps.setTimestamp(3,java.sql.Timestamp.valueOf(fecha.atTime(hora)));ps.setString(4,txtCodigo.getText().trim());ps.setString(5,cmbLaboratorio.getSelectedItem().toString());ps.executeUpdate();}javax.swing.JOptionPane.showMessageDialog(this,"El reporte fue enviado con estado Pendiente.","Reporte registrado",javax.swing.JOptionPane.INFORMATION_MESSAGE);limpiarFormulario();cargarReportes();}catch(java.sql.SQLException ex){javax.swing.JOptionPane.showMessageDialog(this,"No se pudo registrar el reporte:\n"+ex.getMessage(),"Error SQL",javax.swing.JOptionPane.ERROR_MESSAGE);}
    }

    private void verDetalle(){int fila=tablaReportes.getSelectedRow();if(fila<0){javax.swing.JOptionPane.showMessageDialog(this,"Selecciona un reporte.","Sin selección",javax.swing.JOptionPane.WARNING_MESSAGE);return;}fila=tablaReportes.convertRowIndexToModel(fila);int id=((Number)tablaReportes.getModel().getValueAt(fila,0)).intValue();String sql="SELECT f.fecha_reporte,l.nombre laboratorio,COALESCE(i.codigo,i.nombre_equipo,'Sin especificar') equipo,f.descripcion_falla,f.estado,COALESCE(f.observaciones,'Sin observaciones') observaciones FROM reporte_fallas f JOIN laboratorios l ON l.id_laboratorio=f.id_laboratorio LEFT JOIN inventario i ON i.id_inventario=f.id_inventario WHERE f.id_falla=? AND f.id_usuario=?";try(java.sql.Connection con=ConexionBaseDatos.conectar()){if(con==null)throw new java.sql.SQLException("No hay conexión con la base de datos.");try(java.sql.PreparedStatement ps=con.prepareStatement(sql)){ps.setInt(1,id);ps.setInt(2,sesion.getId());try(java.sql.ResultSet rs=ps.executeQuery()){if(rs.next()){String d="Fecha: "+rs.getTimestamp("fecha_reporte")+"\nLaboratorio: "+rs.getString("laboratorio")+"\nEquipo: "+rs.getString("equipo")+"\nEstado: "+rs.getString("estado")+"\n\nDescripción:\n"+rs.getString("descripcion_falla")+"\n\nObservaciones:\n"+rs.getString("observaciones");javax.swing.JOptionPane.showMessageDialog(this,d,"Detalle del reporte",javax.swing.JOptionPane.INFORMATION_MESSAGE);}}}}catch(java.sql.SQLException ex){javax.swing.JOptionPane.showMessageDialog(this,"No se pudo consultar el detalle:\n"+ex.getMessage(),"Error SQL",javax.swing.JOptionPane.ERROR_MESSAGE);}}
    ...
    public static void main(String args[]) {
        try {
            ...
            java.util.logging.Logger.getLogger(VentanaReporteFallaProfesor.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }
        java.awt.EventQueue.invokeLater(() -> new VentanaReporteFallaProfesor("Profesor").setVisible(true));
    }
