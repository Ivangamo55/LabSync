...
public class VentanaReporteFallaProfesor extends javax.swing.JFrame {
    ...
    private void iniciarActualizacionAutomatica() {
        new ActualizacionAutomatica<>(this, 7_000, () -> ConsultaTabla.ejecutar(
                "SELECT id_falla, DATE_FORMAT(fecha_reporte,'%d/%m/%Y') fecha, laboratorio, COALESCE(NULLIF(codigo_equipo,''), nombre_equipo, 'Sin especificar') equipo, estado FROM reporte_fallas WHERE (id_usuario = ? OR (id_usuario IS NULL AND reportado_por = ?)) AND rol_reportante = 'Profesor' ORDER BY fecha_reporte DESC LIMIT 8",
