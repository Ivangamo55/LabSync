...
/**
 * Diseño de la interfaz VentanaReservasAlumno.
 * JFrame Form editable desde NetBeans Swing Designer.
 * Mantiene la maquetación visual del proyecto AplicacionLabSync.
 */
public class VentanaReservasAlumno extends javax.swing.JFrame {
        ...
        try (Connection con = ConexionBaseDatos.conectar()) {
            ...
            try (PreparedStatement psLaboratorios = con.prepareStatement(sqlLaboratorios)) {
                ...
                try (ResultSet rsLaboratorios = psLaboratorios.executeQuery()) {
                    ServicioDisponibilidad servicio = new ServicioDisponibilidad();
    ...
    private void solicitarReserva() {
        ...
        try (con) {
            ...
            ServicioDisponibilidad.ResultadoDisponibilidad disponibilidad
                    = new ServicioDisponibilidad().consultarParaAlumno(
    ...
    private String determinarTurno(String horario) {
        String inicioTexto = horario.substring(0, horario.indexOf(" - ")).trim();
        java.time.LocalTime inicio = java.time.LocalTime.parse(inicioTexto,
                java.time.format.DateTimeFormatter.ofPattern("H:mm"));
        return inicio.isBefore(java.time.LocalTime.of(14, 10)) ? "Matutino" : "Vespertino";
    }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> {
            VentanaReservasAlumno reservas = new VentanaReservasAlumno("Alumno");
